...
void LabelsRender()
{
    ...
    for (auto &entt : view)
    {
        ...
        if (camera.Contains(label.dst_rect))
        {
            ...
            Graphics::RenderToLayer(6, label.texture, &label.src_rect, &label.dst_rect);
        }
    };
}
...
void CreateLabels()
{
    ...
    }
    {
        ...
        for (int i = 0; i < 100; i++)
        {
            textureCache.load(std::to_string(i), ResourceLoader::Text(fontCache.resource("font35"),
    ...
    }
    {
        ...
        textureCache.load("name",
                          ResourceLoader::Text(fontCache.resource("font23"), player.name, {255, 255, 255, 255}));
...
void ResetTable()
{
    ...
    for (auto &entt : view)
    {
        ...
        textureCache.load("table" + std::to_string(i),
                          ResourceLoader::Text(fontCache.resource("font35"), text, {255, 255, 255, 255}));
...
void ResetName()
{
    ...
    for (auto &entt : view_player)
    {
        ...
        textureCache.load("name",
                          ResourceLoader::Text(fontCache.resource("font23"),
